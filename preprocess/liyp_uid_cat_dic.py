...
def liypUidCat():	
#	os.mkdir("../data/dictionary/date_u_c")
	direction = "../data/date/"
	...
	for file_name in file_list:
	    ...
	    for row in rows:
		sample = (row[0], row[4])  # Attention: tuple is hashable,but list is not hashable
		if dictionary.has_key(sample):
		    dictionary[sample][0][int(row[2])-1] += 1
		    if int(row[2]) == 1 or int(row[2]) == 3:			#click and cart   +1
		    	dictionary[sample][0][4] += 1
		    if dictionary[sample][1].count(row[3])==0:
		        dictionary[sample][1].append(row[3])
		else:
		    dictionary[sample]=[[0,0,0,0,0],[row[3]]]
		    dictionary[sample][0][int(row[2])-1] = 1
		# dictionary[sample][3].sort()
	    f.close()
            
	    f = open("../data/dictionary/date_u_c/"+file_name.split('.')[0]+".pkl",'wb')
